...
public partial class depositupload : System.Web.UI.Page
{
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        if (fileExtension.ToLower() == ".jpg" || fileExtension.ToLower() == ".gif" || fileExtension.ToLower() == ".png" || fileExtension.ToLower() == ".bmp")
        {
            ...
            {
                MoneyTransaction photoEntity = new MoneyTransaction();
                photoEntity.ReceiptPhoto = bytes;
                photoEntity.Value =Convert.ToDecimal(inputvalue.Text);                
                photoEntity.CreatedDate = DateTime.Now;
                photoEntity.SourceId = userId;
                     
                var Result =  tempClient.SaveMoneyTransactionDeposit(photoEntity);
                if (Result.IsSuccess && Result.Result != null)
                {
                    lblMessage.Visible = true;
                    lblMessage.ForeColor = System.Drawing.Color.Green;
                    lblMessage.Text = "Upload Successful";
                    uploadedimage.Src="data: image / png; base64," +Convert.ToBase64String(Result.Result.ReceiptPhoto);
                }
            }
        } else {
            lblMessage.Visible = true;
